  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Description", value: "DepartmentDescription" },
  ...
  methods: {
    async initialize() {
      try {
        ...
        this.departments = this.departments.filter(item=>item.DepartmentName != 'None')
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async deleteItemConfirm() {
      try {
        ...
        await axios({
          url: `http://localhost:3000/department/delete?id=${this.editedItem._id}`,
          method: "DELETE",
          headers: {
            "auth-token": token,
          },
    ...
    async save() {
      try {
        ...
        if (this.isAddItem == false) {
          await axios({
            url: `http://localhost:3000/department/update?id=${this.editedItem._id}`,
            data: user,
            method: "PUT",
            headers: {
              "auth-token": token,
